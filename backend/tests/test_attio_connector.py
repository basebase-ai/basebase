...
@pytest.mark.asyncio
async def test_write_create_deal_passes_custom_attribute_slugs(monkeypatch: pytest.MonkeyPatch) -> None:
    c = _connector()
    recorded: list[dict[str, Any] | None] = []
    ...
    assert body is not None
    vals: dict[str, Any] = body["data"]["values"]
    assert vals["pipeline_type"] == "Sales"
    assert vals["priority"] == "P1"
    assert vals["name"] == [{"value": "GAIA"}]


@pytest.mark.asyncio
async def test_write_update_deal_custom_attributes_only(monkeypatch: pytest.MonkeyPatch) -> None:
    c = _connector()
    ...
    await c.write(
        "update_deal",
        {"id": "deal-uuid-1", "pipeline_type": "Renewal"},
    )
    assert recorded == [
        (
            "PATCH",
            "/v2/objects/deals/records/deal-uuid-1",
            {"data": {"values": {"pipeline_type": "Renewal"}}},
        ),
    ]
