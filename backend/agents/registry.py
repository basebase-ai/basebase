...
- temp_data: Agent-generated results and computed metrics. Flexible JSONB storage linked to entities. Columns: entity_type, entity_id, namespace, key, value (JSONB), metadata (JSONB), created_by_user_id, created_at, expires_at. Daily digests use namespace='daily_digest', key=ISO date, entity_type user|team. Example: SELECT td.value FROM temp_data td WHERE td.namespace = 'daily_digest' AND td.key = '2026-05-18' AND td.entity_type = 'user'
...
- temp_data: (entity_type, entity_id, namespace, key, value, metadata, expires_at) → immediate. Flexible JSONB store for computed results. Use namespace to group (e.g. 'daily_digest', 'deal_confidence'). value is JSONB, any shape. Upsert daily digest rows with ON CONFLICT ON CONSTRAINT uq_temp_data_digest_slot DO UPDATE.
...
register_tool(
    name="collect_digest_data",
    description="""Collect raw synced activity for one team member on a single calendar day (America/Los_Angeles).

Returns JSON with activities, meetings, tracker_issues, github_commits, github_pull_requests,
shared_files, active_sources, member_name, org_name, and digest_date. Use before summarizing a
member digest in the Daily Digest workflow.""",
    input_schema={
        "type": "object",
        "properties": {
            "user_id": {
                "type": "string",
                "description": "UUID of the team member to collect data for.",
            },
            "digest_date": {
                "type": "string",
                "description": "Calendar date in YYYY-MM-DD (America/Los_Angeles day).",
            },
        },
        "required": ["user_id", "digest_date"],
    },
    category=ToolCategory.LOCAL_READ,
    default_requires_approval=False,
    status_running="Collecting digest data",
    status_complete="Collected digest data",
)
